...
test.describe("Admissão Preliminar", { tag: ["@ADMISSAO_PRELIMINAR"] }, () => {
    ...
    */
    test("realizar admissão preliminar com sucesso contribuinte dirigente sindical", { tag: "@ADMISSAO_PRELIMINAR_CONTRIBUINTE_SUCESSO" }, async({ page }) => {
        ...
        await page.locator('input[name="fp.nome-STRING-inicial"]').fill(usuario.nome);
        await page.locator('input[name="fp.cpf-STRING-inicial"]').fill(usuario.cpf);
        console.log(usuario.nome);
        await page.locator('input[name="fp.nome-STRING-inicial"]').press('Enter');

        await expect(page.locator('.grid tbody tr:has-text("' + usuario.nome + '")')).toBeVisible();
