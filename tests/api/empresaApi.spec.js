...
test.describe.serial("Empresa API", { tag: ["@EMPRESA_API"] }, () => {
   ...
   test("cadastrar empresa", { tags: ["@EMPRESA_SUCESSO_API"] }, async () => {
        ...
        const gerarEmpresa = apiEmpresaHelpers.gerarEmpresa("CNPJcomMascara");  
        try {
            ...
            expect(response.data).toHaveProperty("cliente_id", expect.anything());
   ...
   test("cadastrar a mesma empresa para clientes diferentes", { tags: ["@EMPRESA_SUCESSO_API"] }, async () => {
        ...
        try {
            ...
            const loginResponseB = await login("testeunitario", "S#nta799");
   ...
   test("cadastrar empresa com documento menor que 14 caracteres", { tags: ["@EMPRESA_FALHA_API"] }, async () => {
        ...
        try {
            ...
            expect(error.response.data.detail[0]).toHaveProperty("msg", MENSAGENS.empresa.textoDocumentoCurto);
        }
   });

   test("cadastrar empresa com documento maior que 18 caracteres", { tags: ["@EMPRESA_FALHA_API"] }, async () => {
        const apiEmpresaHelpers = new ApiEmpresaHelpers();
        const gerarEmpresa = apiEmpresaHelpers.gerarEmpresa("CNPJmais18");
        try {
            const loginResponse = await login("joao.garcia", "Mudar@12");
            const response = await cadastrarEmpresa(gerarEmpresa, loginResponse.data.token);
            throw new Error("Esperava um erro, mas a requisição foi bem-sucedida.");
            ...
            expect(error.response.status).toBe(422);
            expect(error.response.data.detail[0]).toHaveProperty("msg", MENSAGENS.empresa.textoDocumentoLongo);
        }
   });

   test("cadastrar empresa com documento maior que 14 caracteres especiais", { tags: ["@EMPRESA_FALHA_API"] }, async () => {
        const apiEmpresaHelpers = new ApiEmpresaHelpers();
        const gerarEmpresa = apiEmpresaHelpers.gerarEmpresa("CNPJmais15");
        try {
            const loginResponse = await login("joao.garcia", "Mudar@12");
            const response = await cadastrarEmpresa(gerarEmpresa, loginResponse.data.token);
            throw new Error("Esperava um erro, mas a requisição foi bem-sucedida.");
            ...
            expect(error.response.status).toBe(422);
            expect(error.response.data.detail[0]).toHaveProperty("msg", MENSAGENS.empresa.textoDocumentoInvalido);
        }
   });
   ...
   test("cadstrar empresa com token expirado", { tags: ["@EMPRESA_FALHA_API"] }, async () => {
        const apiEmpresaHelpers = new ApiEmpresaHelpers();
        const gerarEmpresa = apiEmpresaHelpers.gerarEmpresa("CNPJcomMascara");  
        try {
